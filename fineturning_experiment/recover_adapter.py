"""Recover meta.json and rewrite adapter.pt in the proper registry format.

Run from the repo root:
    conda run -n boltz_affinity python fineturning_experiment/recover_adapter.py
"""
...
adapter_dir = Path(__file__).parent / "adapters" / "drd4_v2_rank_64"

# ── Load the raw training checkpoint that was copied to adapter.pt ────────────
...
manifest_path = str(
    (Path(__file__).parent / "manifests" / "lora_manifest_DRD4.csv").resolve()
)
run = TrainingRun(
    ...
    notes=(
        "DRD4 LoRA rank=32 alpha=64 — meta.json recovered from checkpoint "
        "after FileExistsError bug (checkpoint dir pre-existed registry.save)"
    ),
)
adapter = LoRAAdapter(
    name="drd4_v2_rank_64",
    config=config,
    history=[run],
)
